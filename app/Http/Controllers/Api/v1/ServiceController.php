...
class ServiceController extends Controller
{
    ...
    public function add_edit_service(Request $request) {
        ...
        $check_added = HairdresserService::where('hairdresser_id',$user->id)->where('category_id',$request->category_id)->first();
        if($check_added) {
            $check_added->price = $request->price;
            $check_added->save();
        } else {
            $new = new HairdresserService;
    ...
    public function delete_service(Request $request) {
        ...
        $check_added = HairdresserService::where('hairdresser_id',$user->id)->where('id',$request->service_id)->delete();
    ...
    public function service_list(Request $request) {
        ...
        $category_list = HairdresserService::where('hairdresser_id',$user->id)->get()->pluck('category_id')->toArray();
...
//        }
        ...
        //pre($service_ids);
        $hairdresser_ids = HairdresserService::groupBy('hairdresser_id')
...
//        pre($request->all()); exit();
        ...
        $order_hairdresser_ids = Order::whereIn('order_status',[ORDER_ACCEPT,ORDER_ON_THE_WAY,ORDER_PROCESSING])->pluck('hairdresser_id')->toArray();
//        $hair_dresser_service = HairdresserService::distinct('hairdresser_id')->pluck('hairdresser_id')->toArray();
...
//        }
        ...
        //$service_ids = json_decode($request->service_ids);
        //pre($service_ids);
        //$hairdresser_ids = HairdresserService::groupBy('hairdresser_id')->having(DB::raw('count(DISTINCT CASE WHEN category_id IN ('.implode(',',$service_ids).') THEN category_id END)'),'=',count($service_ids))->select('hairdresser_id')->get()->pluck('hairdresser_id')->toArray();
        //pre($category_list);
        //$data_record = DB::select("SELECT hairdresser_id FROM hairdresser_services GROUP BY hairdresser_id HAVING COUNT(DISTINCT CASE WHEN category_id IN (1,2) THEN category_id END) = 2");
