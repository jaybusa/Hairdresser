...
class ReportController extends Controller {
    ...
    public function index(Request $request) {
        ...
        $Dressers = User::where('role_id','3')->get();
        if($userId >= 1 ){
            $UserWalletHistory = \App\Models\UserWalletHistory::where('user_id',$userId)->where('created_at','>=',$fromDate)->where('created_at','<=',$toDate)->with('users')->with('order_detail')->orderBy('id','desc')->get();    
        }else{
            $userId = -1;
            $dresserIds = array();
            foreach($Dressers as $_dresser){
